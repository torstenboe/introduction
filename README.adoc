...
The ocloud framework is a collection of provisioning scripts that acclerate the the adoption of enterprise cloud services with link:https://www.oracle.com/cloud/[Oracle Cloud Infrastructure (OCI)]. While provisioning a cloud server has become a matter of pressing a button, launching hosting platforms for intra- and extranet services is more complex. It requires operators to follow security and compliance guidelines that often are in conflict with the operating model of public clouds. This framework aims at overcoming typical obstacles. Employing deployment templates that combine dedicated infrastructure with operator controls into service assets, the framework allows operators to keep control over managed infrastructure. Applying link:https://en.wikipedia.org/wiki/Infrastructure_as_code["Infrastructure as Code" (IaC)] on OCI preserves the flexibility to deploy services on-demand with comprimising on security and compliance requirements. 
