...
Configuration files represent the design, scale and scope of a service. In the refinement phase engineers collect input from practitioners to control demand and optimize capacity utilization, to enable operators to manage service availability, schedule resource consumption and fix problems on a managed infrastructure platform. In addition service configurations enable service manager to adopt Oracle Cloud Services as alternative for shared intranet services, and to ease the development of extranet services with utility computing, web- and mobile commerce services.
