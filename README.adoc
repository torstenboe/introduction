...
Service configuration represents a next level of customization. JSON files represent the design, scale and scope of a service. In the refinement phase teams collect input from practitioners to refine settings that allow operators to controll demand and optimize capacity utilization. An operator is employed to manage service availability, schedule resource consumption and fix problems for highly abstracted infrastructure programmatically. In addition service configurations enable service manager to adopt Oracle Cloud Services as alternative to shared intranet services and to benefit from pltfrom offerings like utility computing, web- and mobile backbone services. The following configuration files form an adoption framework:
