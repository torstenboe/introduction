...
=== Architecture
The default deployment plan defines a link:assets/resident[service resident] in an OCI tenancy. It isolates a private partition in an own network segment with own administrator domains. A domain combines link:https://docs.oracle.com/en-us/iaas/Content/Identity/Tasks/managingcompartments.htm[compartments] with groups, policies, identity tags and notifications. The configuration
