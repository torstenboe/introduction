...
It offers a choice of typical solution topologies for use cases like enterprise applications, big-data and cloud-native services. These options can also be combined to build hybrid services. In the default configuration link:assets/network[network segmentation] is employed, but only one segement, the "core" network is defined. The segement is  separated into subnets, using link:https://www.terraform.io/language/functions/cidrsubnet[Terraform's cidrsubnet() function]. A segment represents a Virtual Cloud Network (VCN) incl. a dynamic routing gateway and gateways for the Oracle Service Network, Internet and NAT zones. Firewalls represent port filter on subnet level and are implmented using security lists. With the default configuration egress traffic remains open, ingress traffic is limited to a minimum set of application profiles and will need adjustments when launching a service. Per defualt an obligatory "core" router is provisioned, gateways can be deactivated using the resource manager interface. Adding network segments to the topology, VCNs can be associated with an own routers by naming both equally.  
