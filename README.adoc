...
=== Architecture
An ocloud  template defines a link:assets/resident[service resident] for an OCI tenancy. IT isolates a private partition with own administrator domains in an own network segment from the generic infrastructure pool represented by a tenancy. A domain combines link:https://docs.oracle.com/en-us/iaas/Content/Identity/Tasks/managingcompartments.htm[compartments] with groups, policies, identity tags and notifications. The default configuration
separates system operators from network-, database-, and security operator.
