...
The objective of a delivery framework for IT assets is empowering IT service manager to align services with the needs of a business. Typically business value is measured in reduced cost, decreased time-to-market or increased revenue. Cloud infrastructure has become a main efficiency leaver, representing resource pools that allow to build IT services faster without upfront investments. But while launching a server has become a matter of pressing a button, preparing a hosting platform for intra- and extranet services remains challenging. Operators have to address security and compliance concerns before provisioning a first resource. This framework enables system administrator to accelerate that process, start with default configuration and refine their platform in small incremental steps. link:https://en.wikipedia.org/wiki/Infrastructure_as_code["Infrastructure as Code" (IaC)] is employed to extract configurations parameters from service assets and to inherit operator controls during the provisioning process. link:https://docs.oracle.com/en-us/iaas/Content/ResourceManager/Concepts/resourcemanager.htm[Oracle's Resource Manager (ORM)] generates graphical user interfaces and REST-API from code variables and enables engineers to build templates that trigger provisioning processes with a service deployment. Service manager benefit from on-demand infrastructure, operators remain in control and effectuate security and comliance guidlines. 
