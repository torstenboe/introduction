...
The framework enables a process that splits the design of a cloud services into three phases:
...
- *Refine*: Refine configuration parameter together with subject matter experts. All configuration parameters are separated into JSON file. This collection delivers for an adoption process that incorporates subject matter experts like network and security operators into a cloud adoption project without teaching infrastructure coding. 
- *Compose*: The last step is service definition, after refining the topology, services can be deployed, either by attaching services from the Oracle Service Network, deploying predefined or developing new service modules.
