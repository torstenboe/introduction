...
==== Service Resident
A link:assets/resident[service resident] isolates a private partition from the infrastructure pool represented by a tenancy. link:https://docs.oracle.com/en-us/iaas/Content/Identity/Tasks/managingcompartments.htm[compartments] separate administrator domains and implements policies for administrator roles.
