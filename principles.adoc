...
* *Build services with a common foundation* Provisioning scripts for service assets ensure secure and compliant application deployments without prebuild resources. This provides a high level of flexibility. The develop once, use often concept is reflected in IaC libraries that can be invoked in a modular fashion. 
