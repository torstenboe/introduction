...
* *Build delivery platforms for services* The foundation of a service is assembled with reusable assets. This ensures compliance and security with a high level of flexibility. The build once, use often method is reflected in asset libraries that can be invoked in a modular fashion. 

Designing a service, architects focus on system requirements, communication pattern and orchestration capabilities before considering functional requirements. This turns the usual design process upside down and requires to focus on a minimum set of attribtues first.
